...
import { trackFloatingMenu } from "./menus.js";
...
function parseState(type, value) {
  const now = new Date();
  const st = {
    y: now.getFullYear(),
    mo: now.getMonth(),
...
export function createDatePicker({ type, value = "", onCommit, onCancel }) {
  ...
  function positionPopup() {
    ...
    if (top + ph > window.innerHeight - 8 && rect.top - ph - 4 > 8) {
      top = rect.top - ph - 4; // flip above when it would overflow the viewport
    }
    let left = rect.left;
